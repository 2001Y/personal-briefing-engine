...
- approval/action logs now support minimal `approve-action` / `reject-action` state transitions, with basic validation for unknown action IDs and non-pending actions; `complete-action` and `failed-action` can mark approved actions as executed or failed once they are awaiting execution completion; richer side-effect execution updates remain future work
