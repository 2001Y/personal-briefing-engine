...
- approval/action logs now support minimal `approve-action` / `reject-action` state transitions, with basic validation for unknown action IDs and non-pending actions; `complete-action` can also mark approved actions as executed; richer side-effect execution updates remain future work
