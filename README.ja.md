...
- optional な SQLite state wiring は trigger runs / deliveries / delivered-item suppression history / audit-derived feedback logs / `action_prep` 向け approval/action logs / X signal connector cursors / source registry state snapshots（`last_poll_at` / `last_seen_item_ids` / `last_promoted_item_ids` / `authority_tier`）まで入り、より豊かな source registry state（trust-review notes・error metadata 更新など）は今後の実装対象
