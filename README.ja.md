...
- `location-walk` event trigger CLI（`location-dwell` は互換 alias として維持）
...
- 高頻度な位置通知は、Dawarich のような local_store を 5分おき程度で narrow に poll する `location.walk` を想定し、cooldown / suppression で過通知を防ぐ。`location-dwell` は CLI 互換 alias として残す
