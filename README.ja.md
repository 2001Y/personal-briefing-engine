...
- optional な SQLite state wiring は trigger runs / deliveries / X signal connector cursors / basic observed source registry state snapshots（`last_poll_at` / `last_seen_item_ids` / `authority_tier`）まで入り、suppression / feedback persistence やより豊かな source registry state は今後の実装対象
