...
- optional な SQLite state wiring は trigger runs / deliveries / delivered-item suppression history / audit-derived feedback logs / `action_prep` 向け approval/action logs と minimal な approve/reject/complete/failed transition・strict な command/flag validation・receipt/error/provider/store/order metadata 向け structured execution-details 永続化・action execution feedback logging / X signal connector cursors / source registry state snapshots（`last_poll_at` / `last_seen_item_ids` / `last_promoted_item_ids` / `authority_tier`）に加えて、digest delivery での same-trigger suppression filtering まで入り、より豊かな source registry state（trust-review notes・error metadata）や deeper な action execution / feedback loop は今後の対象
